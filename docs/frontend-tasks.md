...
**구현 상태**: 프론트엔드 연결 완료. 데이터 범위·사용자 가치·개인정보 보호 기준은 [벤치마크 기능 정의](benchmark.md)를 기준으로 한다. 고객사 익명 집계 데이터를 활용하는 확장안은 [2026년 8월 4일 회의 검토안](benchmark-expansion-meeting-2026-08-04.md)에서 별도 논의한다.
...
## 핵심 피드백 완료 후 후속 작업·논의

### 대시보드 검색을 공급망 탐색형 검색으로 개선

**상태**: 핵심 피드백 작업을 모두 마친 뒤 진행.

- 품목명/HS 코드 검색: 동일한 품목으로 묶어 `품목명 · HS 코드`를 함께 표시하고, 최고 SGRI·관련 공급국을 요약.
- 국가명/국가코드 검색: 해당 국가와 연결된 **내 등록 품목**을 품목명·HS 코드·해당 국가 SGRI와 함께 표시.
- 품목 결과 클릭: `/risks/{hs_code}` 또는 `/recommendations?query_id={id}`로 이동.
- 국가 결과 클릭: 관련된 내 등록 품목 목록을 보여주고, 선택한 품목의 리스크 상세로 이동.
- 현재 범위는 `api.getQueries()` + `api.getRisks()` 기반의 **내 모니터링 포트폴리오**로 제한하며 모든 목록·점수는 API 응답을 사용.
- 전체 글로벌 품목 탐색은 별도 사용자 스토리와 데이터·검색 API가 필요한 기능이므로, 필요성이 확인되면 `/explore` 또는 `/global-items` 같은 독립 화면으로 검토.

### 부정 피드백 이후 실제 재추천 정책

**현재 상태**: `도움이 안 됐어요`는 `POST /feedback`에 `rating: -1`을 저장할 뿐 순위를 재계산하지 않음.

**추가 논의 필요**:
- 기존 1위 국가를 자동 제외할지 여부.
- 사용자가 제외 국가를 직접 선택하게 할지 여부.
- 가격·납기·위험도 등의 조건을 변경한 뒤 재추천할지 여부.
- 실제 재추천을 위한 신규 백엔드 API와 요청 형식.
