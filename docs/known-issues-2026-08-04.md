...
> `feature/frontend`는 `SECRET_KEY: ${SECRET_KEY:-dev-secret-change-in-production-min-32-bytes!}`로
> 기본값 폴백을 뒀습니다. 그대로 가져오면 "전달은 되지만 값은 여전히 기본값"이 될 수 있으니,
> 위처럼 `:?`(미설정 시 컨테이너 기동 실패)로 바꾸거나 최소한 운영 배포에서는 반드시 주입되게 하는 편이 안전합니다.
> Railway 등 compose를 쓰지 않는 배포 환경은 별도로 주입 여부를 확인해야 합니다.
