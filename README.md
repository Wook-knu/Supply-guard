...
**AI 기반 공급망 리스크 관리 플랫폼** — 공개 데이터를 모아 품목·국가별 공급망 위험을 0~100점(SGRI)으로 보여 주고, 대체 공급국·거래 기업 추천부터 대응 보고서 초안까지 하나의 흐름으로 이어 주는 중소·중견기업용 서비스입니다.
...
> 📌 관세청 수입통계 등 API 키가 필요한 데이터 소스는 키 확보 후 S·V 지표에 추가 도입할 예정입니다.
...
- **백엔드**: FastAPI, SQLAlchemy 2.0, Pydantic v2 · *Railway 배포*
- **데이터베이스**: PostgreSQL
- **AI**: Google Gemini
...
> 백엔드는 시작 시 필요한 스키마를 자동으로 보정하고 배터리 소재 실기업 시드를 1회 로드합니다.
...
| `GEMINI_API_KEY` | ✅ | 가중치·AI 설명·보고서 생성용 |
| `SECRET_KEY` | ✅(운영) | JWT 서명키 |
| `GOOGLE_CLIENT_ID` | 선택 | 없으면 구글 로그인 비활성 |
| `NEXT_PUBLIC_API_BASE_URL` | ✅(배포) | 프론트가 호출할 백엔드 주소 |
| `COMTRADE_API_KEY` 등 | 선택 | 신규 품목 실데이터 수집용 |

> 무료·무키 API(World Bank·GDELT·GDACS·IMF PortWatch·FRED 공개 CSV)만으로도 기본 동작이 가능합니다.
