...
// 신규 모니터링 품목을 입력·검증·임시 저장하는 화면입니다.
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <main className="mx-auto max-w-6xl px-5 py-8 md:px-8">
        ...
        <div className="mt-6 flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><div className="mb-2 flex items-center gap-2 text-sm font-medium text-blue-600"><PackagePlus className="h-4 w-4" /> 공급망 등록</div><h1 className="text-2xl font-semibold tracking-tight md:text-3xl">모니터링할 품목을 등록하세요</h1><p className="mt-2 text-sm text-slate-500">품목과 조달 조건을 입력하면 AI가 공급망 위험도를 분석합니다.</p></div><Badge className="w-fit border-blue-100 bg-blue-50 px-3 py-1.5 text-blue-700 hover:bg-blue-50">1 / 2 기본 정보</Badge></div>

        <div className="mt-8 grid gap-6 lg:grid-cols-3">
          <form id="item-form" onSubmit={saveItem} className="lg:col-span-2"><Card className="border-slate-200 shadow-sm"><CardHeader className="border-b border-slate-100 pb-5"><CardTitle className="text-base">품목 및 조달 정보</CardTitle><CardDescription className="mt-1">* 표시는 필수 입력 항목입니다.</CardDescription></CardHeader><CardContent className="space-y-7 pt-6">
