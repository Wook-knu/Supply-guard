...
import { ArrowLeft, ArrowRight, BadgeCheck, Building2, Check, ExternalLink, Globe2, MapPin, ShieldAlert, ShieldCheck, Truck } from "lucide-react"
      ...
      {!loaded ? <p className="mt-16 text-center text-sm text-slate-400">불러오는 중…</p>
        ...
        <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
          ...
          <div className="mt-7 flex flex-wrap items-center gap-3 border-t border-slate-100 pt-5">
            ...
            <Button asChild className="bg-blue-600 hover:bg-blue-700"><Link href={queryId ? `/reports/new?query_id=${queryId}` : "/reports/new"}>보고서 초안 만들기 <ArrowRight className="ml-2 h-4 w-4" /></Link></Button>
          </div>
        </section>
